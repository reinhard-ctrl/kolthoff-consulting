...
(function (global) {
  ...
  function buildSopFromWorkflowTab(tab, profile) {
    ...
    return {
      ...
      objective: `Complete the ${tab.name} process correctly and on time.`,
      scope: `Applies to ${uniqueLanes.join(', ') || 'the roles shown in this workflow'}.`,
      prerequisites: '',
      kpis: '',
      troubleshooting: '',
      raci: {
        responsible: rRoles.join(', ') || uniqueLanes.join(', ') || 'Process owners',
        accountable: aRoles.join(', ') || uniqueLanes[0] || 'Manager',
        consulted: cRoles.join(', ') || '',
        informed: iRoles.join(', ') || '',
      },
      steps: mappedSteps,
