...
(function (global) {
  ...
  function renderAddendumPrintDocument(props) {
    ...
    return React.createElement(
      'div',
      { id: 'addendum-print-area', className: 'xl:col-span-8 planner-split-panel print:w-full space-y-8 print:space-y-0 text-left font-sans' },
      React.createElement(
        'section',
        { className: 'print-card bg-white text-slate-900 p-8 rounded-xl border border-slate-200 print:rounded-none print:border-0 print:p-0 print:break-after-page' },
        React.createElement(
          'div',
          { className: 'space-y-6 min-h-[70vh] flex flex-col justify-between' },
          React.createElement(
            'div',
            { className: 'space-y-4' },
      ...
      React.createElement(
        'section',
        { className: 'print-card bg-white text-slate-900 p-8 rounded-xl border border-slate-200 print:rounded-none print:border-0 print:p-0 print:break-after-page' },
        React.createElement('h2', { className: 'text-sm font-mono uppercase tracking-wider text-slate-400 font-bold mb-4' }, 'Additional deliverables'),
        React.createElement(
          'div',
          { className: 'space-y-4' },
          selectedTasks.map((task) => React.createElement(
            'article',
            { key: task.id, className: 'border border-slate-200 rounded-lg p-4 print-avoid-break' },
            React.createElement('h3', { className: 'text-sm font-bold text-slate-900' }, task.deliverable),
            React.createElement('p', { className: 'text-[10px] font-mono text-slate-500 mb-2' }, task.category),
            React.createElement('p', { className: 'text-xs text-slate-700' }, task.description),
            task.scopeDetails?.output && React.createElement('p', { className: 'text-xs text-slate-600 mt-2' }, React.createElement('strong', null, 'Output: '), task.scopeDetails.output),
          )),
        ),
      ),
      React.createElement(
        'section',
        { className: 'print-card bg-white text-slate-900 p-8 rounded-xl border border-slate-200 print:rounded-none print:border-0 print:p-0' },
        React.createElement('h2', { className: 'text-sm font-mono uppercase tracking-wider text-slate-400 font-bold mb-4' }, 'Addendum quotation (separate invoice)'),
        React.createElement(
          'table',
          { className: 'w-full text-xs border-collapse mb-4' },
          React.createElement('tbody', null,
            React.createElement('tr', { className: 'border-b border-slate-200' },
              React.createElement('td', { className: 'py-2 font-bold' }, 'Addendum professional fees'),
              React.createElement('td', { className: 'py-2 text-right font-mono' }, formatCurrency(totalBase)),
            ),
            includeTax && React.createElement('tr', { className: 'border-b border-slate-200' },
              React.createElement('td', { className: 'py-2' }, '12% VAT'),
              React.createElement('td', { className: 'py-2 text-right font-mono' }, formatCurrency(vat)),
            ),
            React.createElement('tr', null,
              React.createElement('td', { className: 'py-2 font-bold' }, 'Total due (this addendum)'),
              React.createElement('td', { className: 'py-2 text-right font-mono font-bold' }, formatCurrency(total)),
            ),
          ),
        ),
        (addendumEconomics.billingMilestones || []).length > 0 && React.createElement(
          'div',
          { className: 'mb-4' },
          React.createElement('p', { className: 'text-[10px] font-mono uppercase text-slate-400 font-bold mb-2' }, 'Payment schedule (this addendum only)'),
          React.createElement(
            'ul',
            { className: 'text-xs space-y-1' },
            addendumEconomics.billingMilestones.map((m, i) => React.createElement(
              'li',
              { key: i, className: 'flex justify-between border-b border-slate-100 py-1' },
              React.createElement('span', null, m.label),
              React.createElement('span', { className: 'font-mono' }, formatCurrency(m.amount)),
            )),
          ),
        ),
        React.createElement('p', { className: 'text-[10px] text-slate-600 leading-relaxed border-t border-slate-200 pt-3' }, addendum.addendumTerms),
        React.createElement('p', { className: 'text-[10px] text-slate-500 mt-4 italic' }, 'This addendum is billed separately from the original Statement of Work. Original engagement milestones are unchanged.'),
